...
| Yellow cards, red cards and substitution counts | ESPN or FotMob when bilateral values/events exist | Red-card difference adjusts corner share by at most 8%; other counts are retained but not assumed tactical |
...
- Keep source attribution (`ESPN` or `FotMob`) with live attacking and pressure metrics.
